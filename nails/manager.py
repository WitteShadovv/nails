...
from .exceptions import NailsError, NailsPermissionError, NailsStateError
...
class NailsManager:
    ...
    def activate(self):
        ...
        if not self.config_manager.config_exists():
            raise NailsStateError("Hidden configuration not found. Run './nails.py init' first")
    ...
    def rebuild(self):
        ...
        if not self.state_manager.is_active():
            raise NailsStateError("Hidden environment is not active. Run './nails.py activate' first")

        if not self.config_manager.config_exists():
            raise NailsStateError("Hidden configuration not found at config/configuration.nix")
    ...
    def status(self):
        ...
        print(f"Overlay structure: {'Present' if self.overlay_manager.structure_exists() else 'Missing'}")
        print(f"Hidden config: {'Present' if self.config_manager.config_exists() else 'Missing'}")
