...
class OverlayManager:
    ...
    def create_mounts(self):
        ...
        for target in self.overlay_targets.keys():
    ...
    def activate(self):
        ...
        try:
            ...
            raise NailsOverlayError(f"Failed to activate overlays: {e}")

    def _mount_overlay(self, target: str, overlay_path: Path):
        ...
        if target == "/home":
            print("  This prevents leakage of user data, shell history, and application configs")
        
        cmd = [
            "mount", "-t", "overlay", "overlay",
            "-o", f"lowerdir={target},upperdir={overlay_path},workdir={work_path}",
            target
        ]

        result = subprocess.run(cmd, capture_output=True, text=True, timeout=30)
        if result.returncode != 0:
            raise NailsOverlayError(f"Failed to create {target} overlay: {result.stderr}")
    ...
    def _terminate_overlay_processes(self):
                ...
                for line in lsof_result.stdout.split('\n')[1:]:
                    if line.strip():
                        ...
                        if len(parts) >= 2 and parts[1].isdigit():
                            pid = parts[1]
                            command = parts[0]
                            if command not in ['systemd', 'kernel', 'python3', 'nails.py']:
    ...
    def _unmount_overlays_with_retry(self):
        ...
        for attempt in range(max_retries):
            ...
            for target in targets:
                try:
                    result = subprocess.run(["umount", target], 
                                          capture_output=True, text=True, timeout=10)
                    if result.returncode == 0:
                        print(f"    ✓ {target} overlay unmounted")
                    else:
                        print(f"    ⚠️ Failed to unmount {target}: {result.stderr.strip()}")
                        success = False
                        
                        # Try lazy unmount
                        lazy_result = subprocess.run(["umount", "-l", target], 
                                                   capture_output=True, text=True)
    ...
    def get_mount_status(self) -> Dict[str, str]:
        ...
        for target in self.overlay_targets.keys():
            try:
                result = subprocess.run(["findmnt", target], 
                                      capture_output=True, text=True)
